...
##Coloured figure
persp3D(x_log10kf,y_log10kcat,xlab="log10 kf",ylab="log10 kcat",zlab="Equilibrium flux (M/s)",zlim=c(0,10^-6),P_Phi_eq[[1]],theta = -20, phi = 18, expand = 0.8
...
persp3D(x_log10kf,y_log10kcat,zlim=c(0,10^-6),P_Phi_eq[[1]],theta = -20, phi = 20, expand = 0.8
        ,clab = c("Flux(M/s)"),shade=0.25,border="black",axes = FALSE, ticktype="simple",ticks = FALSE,col=palet,#palet[P_cut]
        box=TRUE,cex.lab=1,cex.axis=0.8,r=5,lwd=0.35,lty=1,legend.shrink=0.4,colkey = list(plot = FALSE))
...
multiplePlot("kr=","","Vm=","M/s",kr_set,Vm,ncol,log10kf_var,log10kcat_var,w_kf_FD_generic,
             abs="log10 kf",ord="log10 kcat",palette=pal,image=TRUE,scale="AUTO",labcex=1,legcex=1,axcex=1,colorkey=FALSE) ##The unit and the value of Vm need to be checked and explained
...
multiplePlot("","","","",Affinity_set,Flux_set,ncol,log10kf_var,log10kcat_var,w_kf_FD_sens,
             ...
             labcex=1.5,TEXT_to_Add=addtxt,cextext=1.5,meth = "edge",legcex=0.575,axcex=0.7,colorkey="COMMON")
...
contour(w_kf_FD_sens_sat[[1]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=1,lty=1,method = "edge",xlim=c(0,1))
title(main="",outer=FALSE,line=2.5,cex.main=1.5,font=2,xlab="log10 (kf)",ylab="log10 (kcat)",cex.lab=1)
for (l in 1:(length(Vm_set))){
  for (s in 1:length(Se_set)){
  contour(w_kf_FD_sens_sat[[(l-1)*length(Se_set)+s]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=s,lty=l+0,method = "edge",add=TRUE,xlim=c(0,1))
...
legend("bottomleft",title=c("Saturation level         Affinity            "),legend=sublegend,lty=c(rep(1,3),1,2),col=c(2,1,3,1,1),ncol=2,cex=0.7)
...
N_reso=50
#Defining parameters
Vm_set=c(10^-6,10^-6) #unit: M.s^-1, with Vm=5.10^-5pmol.s^-1.cell^-1 and Vcell=50 micrometers^3
KT_set=c(5*10^-5)#
log10kf_var<-c(0,10,2) #forward preferred to 1 as forward is already an emerging constant from diffusion and association
log10kcat_var<-c(-4,6,2) #catalytic preferred to -1 for same reasons
kf<-10^seq(log10kf_var[1],log10kf_var[2],length.out=N_reso)
...
palet<-jet.colors(ncol)
pal<-list(palet)
addtxt<-list(l=0.95,h=0.95,txt=c("A","B"),srt = 0,font=2,col="black")

options(digits=3)
deg=round(eta_d_set*Vm_set[1],5)
round(deg,5)
options(digits=10)
subtitle<-list(substitute(paste("Low degradation rate - ", eta[d]," = ",10^{-4},"/s and irreversible first reaction"),list(deg=deg[1])),
               substitute(paste("High degradation rate - ", eta," =",eta_d,"VTm, irreversible first reaction"),list(eta_d=eta_d_set[2])))
               
multiplePlot("KT=","","Vm=","M/s",KT_set[1],signif(Vm_set[1],4),ncol,log10kf_var,log10kcat_var,list(w_kf_FD_sens_2Enz[[1]]),
             abs="log10 (kf)",ord="log10 (kcat)",ncont=5,palette=pal,image=TRUE,scale="AUTO",labcex=1.25,sub=subtitle,subcex=2,TEXT_to_Add=addtxt,meth="flattest")

multiplePlot("KT=","","Vm=","M/s",KT_set,signif(Vm_set,4),ncol,log10kf_var,log10kcat_var,Conc_prod_eq_sens_2Enz,
             abs="log10 (kf)",ord="log10 (kcat)",ncont=10,palette=pal,image=TRUE,scale="AUTO",labcex=1.5,sub=subtitle,subcex=1)
...
contour(w_kf_FD_sens_2Enz_background_eff[[1]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=1,lty=1,method ="edge",xlim=c(0,1))
contour(w_kf_FD_sens_2Enz_background_eff[[2]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=2,lty=1,method ="edge",xlim=c(0,1),add=TRUE)
  ...
  for (l in 2:(length(eta_d_set))){
    for(s in 1:length(kcat1_set)){
      contour(w_kf_FD_sens_2Enz_background_eff[[(l-1)*length(kcat1_set)+s]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=s,lty=l+1,method = "edge",add=TRUE,xlim=c(0,1))
...
legend(0.75,0.85,title="Degradation rate",legend=sublegend,bty="n",lty=c(1,3),col=c(1),ncol=1,cex=0.7)
...
multiplePlot("kr=","","Vm=","M/s",kr,Vm[[1]],ncol=128,log10kcatKM_var,log10kcat_var,w_kcatKM,
             abs="log10 kcat/KM",ord="log10 kcat",scale=c(0,1),lev=c(0.9),
             xyData=Cat_Data_list,TEXT_to_Add =addtxt1,palette=pal,image=TRUE,ncat=4,pcex=1,subcex=1,labcex=2,legtext = legtext1,legpos=c(1.09,0.5),axcex=1.25,cextext=2,legtitle="Enzyme classes")
...
multiplePlot("kr=","","Vm=","M/s",KT,Vm[1],ncol=128,log10kcatKM_var,log10kcat_var,w_kcatKM,
             ...
             xyData=Evo_data,TEXT_to_Add =addtxt1,palette=pal,image=TRUE,ncat=4,pcex=0.75,subcex=1,labcex=2,legtext = legtext1,legpos=c(1.09,0.5),axcex=1.25,cextext=2,legtitle="log10(Ne)",col_data = c(1,2,3,4),pch_dat=c(rep(19,4)))
...
text(0.425,0.95,"0.99",srt=-90,font=1,col=1,cex=2,family="serif")
text(0.525,0.94,"0.999",srt=-90,font=1,col=2,cex=2,family="serif")
text(0.625,0.9275,"0.9999",srt=-90,font=1,col=3,cex=2,family="serif")
text(0.725,0.9175,"0.99999",srt=-90,font=1,col=4,cex=2,family="serif")
...
for (l in 1:(length(KT_set))){
  for (s in 1:(length(Keq_set_sens)+1)){
    w_kf_FD_sens_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]<-round(P_Phi_eq_sens_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]/max(P_Phi_eq_sens_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]),digits=20)
  }
}
...
palet<-jet.colors(ncol)
pal<-list(palet)
addtxt<-list(l=0.95,h=0.95,txt=c("A","B","C"),srt = 0,font=2,col="black")
addtxt11<-list(l=0.75,h=0.2,txt=expression(paste("Diffusion limit ",DL[2])),srt = 45,font=3,col="black",cex=1)
...
multiplePlot("kr=","","Vm=","M/s",KT_set,K_eq_ref,ncol=128,log10kcatKM_var,log10kcat_var,list(w_kf_FD_sens_2Enz_rev[[1]]),
             ...
             pcex=0.75,subcex=1,axcex=1.25,cextext=2,labcex=2)

for (s in 2:3){
  contour(w_kf_FD_sens_2Enz_rev[[s]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=2,lty=5-s,method = "edge",add=TRUE,xlim=c(0,1))
}
for (s in 4:5){
  contour(w_kf_FD_sens_2Enz_rev[[s]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=3,lty=s-2,method = "edge",add=TRUE,xlim=c(0,1))
}

contour(w_kf_FD_sens_2Enz_rev[[1]],xaxt="n",yaxt="n",levels=c(0.9),labcex=1,col=1,lty=1,method = "edge",add=TRUE,xlim=c(0,1))

text(addtxt$l,addtxt$h,addtxt$txt[2],srt=addtxt$srt,font=addtxt$font,col=addtxt$col,cex=2)
...
addtxt12<-list(l=0.8,h=0.18,txt=expression(paste("(kr=100",s^{-1},")")),srt = 45,font=3,col="black",cex=1)
addtxt13<-list(l=1.06,h=0.75,txt=expression(paste(DL[3],"*")),srt = 45,font=3,col="black",cex=1)
addtxt14<-list(l=1.05,h=0.85,txt=expression(paste(DL[4])),srt = 45,font=3,col="black",cex=1)
addtxt2<-list(l=0.05,h=0.95,txt=c("A","B"),srt = 0,font=2,col="black",cex=1)


abline(a=-0.3,b=1,col="grey",lty=6,lwd=1)
abline(a=-0.2,b=1,col="grey",lty=6,lwd=1)
...
title(main=expression(paste("Variable dissociation rates ",k[r]))
      ,col.main="black",font.main=1,cex.main=2)
...
multiplePlot("kr=","","Vm=","M/s",kcat_length_empty,Vm_length_empty,ncol=128,log10kcatKM_var,log10kcat_var,list(w_Etot_kcatKM[[2]]),
             abs="log10 (kcat/KM)",ord="log10 (kcat)",scale=c(0,1),lev=c(0.9),meth="flattest",TEXT_to_Add =addtxt11,palette=pal,image=TRUE,
             pcex=0.75,subcex=1,axcex=1.25,cextext=2,labcex=2)
...
par(mfrow=c(1,1),oma=c(0,0,0,0),mai = c(0.75, 1, 0.75, 0.75))
plot.new()
...
#12.Analyzing with kcat[E]tot for the first enzyme
...
log10kcatKM_var<-c(0,10,2)
kcatKM<-10^seq(log10kcatKM_var[1],log10kcatKM_var[2],length.out=N_reso)
kcat2_set=c(10^-2,10^1,10^4) #Quite efficient catalytic property for the enzyme
  ...
  for (s in 1:(length(Keq_set_sens))){
    ...
    for (i in 1:(N_reso)){
      for (j in 1:N_reso){
        print(paste(l,s,i,j))
        kr2=kr2_set[1]
        kf2=kcatKM2[i]*(1+kr2/kcat2)
        E2_conc=E2_conc_set[j]
        if(kf2/10^10>1){
          tab_P_Phi_eq[i,j]=-10^-20
        }
        else{
    ...
    P_Phi_eq_sens_2Enz_rev[[(s)+length(Keq_set_sens)*(l-1)]]<-tab_P_Phi_eq
    Conc_prod_eq_sens_2Enz_rev[[(s)+length(Keq_set_sens)*(l-1)]]<-tab_Conc_prod_eq
  }
}

#Analysing the results
##Calculating the relative fitness with regards to the maximum achievable flux
w_kcatKM_2Enz_rev<-list()
for (l in 1:(length(kcat_set))){
  for (s in 1:(length(Keq_set_sens))){
    w_kcatKM_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]<-round(P_Phi_eq_sens_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]/max(P_Phi_eq_sens_2Enz_rev[[s+length(Keq_set_sens)*(l-1)]]),digits=20)
  }
}

multiplePlot("KT=","","Vm=","M/s",kcat_set,Vm,ncol,log10kcatKM_var,log10E_tot_var,w_kcatKM_2Enz_rev,
             abs="log10 (kf)",ord="log10 (kcat)",lev=c(0.9),palette=pal,image=TRUE,scale=c(0,1),labcex=1.5,meth = "flattest",subcex=1)
