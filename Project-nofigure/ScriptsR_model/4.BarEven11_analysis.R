...
plot(enzyme_data$log10kcat~enzyme_data$log10KM,pch=20,col="blue",
     xlab="log10(KM)",
     ylab="log10(kcat)",
     main="Trade-off between kcat and KM")
abline(coef=c(toff1$coefficients[1],toff1$coefficients[2]),lwd=2,col="red")
plot(enzyme_data$log10KM~enzyme_data$log10kcat,pch=20,col="blue",
     xlab="log10(KM)",
     ylab="log10(kcat)",
     main="Trade-off between kcat and KM")
abline(coef=c(toff2$coefficients[1],toff2$coefficients[2]),lwd=2,col="red")
...
toff_ortho1 <- deming(enzyme_data$log10kcat~enzyme_data$log10KM)#with package deming
toff_ortho1 <- deming(enzyme_data$log10KM~enzyme_data$log10kcat)#with package deming
print(toff_ortho1)#Very significant relationship, nearly 1:1, seems excessive, reality probably amidst 2 models
abline(coef=c(toff_ortho1$coefficients[1],toff_ortho1$coefficients[2]),lwd=2,col="red")

##a.kcat vs KM trade-off on log-scale
toff2<-lm(enzyme_data$log10kcat~enzyme_data$log10kcat_KM)
summary(toff2)

plot(enzyme_data$log10kcat~enzyme_data$log10kcat_KM,pch=20,col="dark blue",
     xlab="log10(kcat/KM)",
     ylab="log10(kcat)",
...
j=1
for (i in levels(factor(enzyme_data$module_Type))){
        points(enzyme_data$log10kcat[enzyme_data$module_Type==i]~enzyme_data$log10kcat_KM[enzyme_data$module_Type==i],pch=20,col=j)
        col_graph=c(col_graph,j)
        leg_graph<-c(leg_graph,i)
        j=j+1
}
...
abline(coef=c(toff_AA$coefficients[1],toff_AA$coefficients[2]),lwd=2,col=1)
abline(coef=c(toff_CEM$coefficients[1],toff_CEM$coefficients[2]),lwd=2,col=2)
abline(coef=c(toff_INTER$coefficients[1],toff_INTER$coefficients[2]),lwd=2,col=3)
abline(coef=c(toff_SEC2$coefficients[1],toff_SEC2$coefficients[2]),lwd=2,col=4)
