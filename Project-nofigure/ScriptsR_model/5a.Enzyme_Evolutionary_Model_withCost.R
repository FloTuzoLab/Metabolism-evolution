...
#Flux determination
fit<-function(kf,kr,kcat,E_conc,cost_Enz){
    ...
    #Resident phenotype
      ...
      for (t in 1:G){
        ...
        for (i in 1:length(Cell_tab[["N"]])){
          ...
          #}
          Cell_tab[["fit"]][i]<-fit(Cell_tab[["kf"]][i],kr_i,Cell_tab[["kcat"]][i],Cell_tab[["Econc"]][i],cost)
        }
...
#}
  if(mut_bias=="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_bi0",3,"EnzVarCostONLY.rda",sep=""))
  }
  if(mut_bias!="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_co1_",cost^-1,"EnzVarCostONLY.rda",sep=""))
  }
