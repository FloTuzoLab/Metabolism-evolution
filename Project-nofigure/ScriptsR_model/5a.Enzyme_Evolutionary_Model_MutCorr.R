...
if(mut_bias=="increasing"){
  ...
  G<-10^2*Ne##2.5*10^2#In principle 10^3
  mu<-10^-1/Ne
  Sigma_mut<-0.3
  Rho_mut=0.5
  Sigma_mut_corr <- matrix(c(Sigma_mut^2,Rho_mut*Sigma_mut^2,Rho_mut*Sigma_mut^2,Sigma_mut^2),2,2)#VarCov Matrix for mutations affecitng enzyme efficiencies
    ...
    #Resident phenotype
      ...
      for (t in 1:G){
        ...
        for (i in 1:length(Cell_tab[["N"]])){
          Cell_tab[["fit"]][i]<-fit(Cell_tab[["kf"]][i],kr_i,Cell_tab[["kcat"]][i],E_tot_conc)
        }
        if(Cell_tab[["fit"]][i]>10^-3){
          print(paste(Cell_tab[["fit"]][i],Cell_tab[["kf"]][i],Cell_tab[["kcat"]][i]))
        }
      ...
      ##Relative fitness calculation
        ...
        for (i in 1:pop_Size_prev){
          if(mut_table[i]>0){
            ...
            for (m in 1:mut_table[i]){
              if(mut_bias=="increasing"){
                mut_delta_keff<-mvrnorm(n = 1, c(-0.2+mut_bias_coeff*log10(Cell_tab[["kcat"]][i]/10^-4),
                                                   -0.2+mut_bias_coeff*log10(Cell_tab[["kf"]][i])), Sigma_mut_corr)
                ...
                Cell_tab[["kf"]][length(Cell_tab[["N"]])+1]<-10^(log10(Cell_tab[["kf"]][i])+mut_delta_keff[2])
              }
              else{
                mut_delta_keff<-mvrnorm(n = 1, rep(mut_bias,2), Sigma_mut_corr)
                Cell_tab[["kcat"]][length(Cell_tab[["N"]])+1]<-10^(log10(Cell_tab[["kcat"]][i])+mut_delta_keff[1])
                Cell_tab[["kf"]][length(Cell_tab[["N"]])+1]<-10^(log10(Cell_tab[["kf"]][i])+mut_delta_keff[2])
              }
        ...
        #}
        if(t==G/10*g){
          print(paste(Cell_tab[["kcat"]][which(Cell_tab[["N"]]==max(Cell_tab[["N"]]))],Cell_tab[["kf"]][which(Cell_tab[["N"]]==max(Cell_tab[["N"]]))],Cell_tab[["fit"]][which(Cell_tab[["N"]]==max(Cell_tab[["N"]]))]))
...
#}
  if(mut_bias=="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_bi0",3,"PosMutCorr.rda",sep=""))
  }
  if(mut_bias!="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_bi0",-mut_bias*10,"PosMutCorr.rda",sep=""))
  }
