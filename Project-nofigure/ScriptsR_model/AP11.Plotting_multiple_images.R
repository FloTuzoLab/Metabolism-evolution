...
multiplePlot<-function(nvar1,unitV1,nvar2,unitV2,v1,v2,ncol,xlim,ylim,xyZ,tit,
                       abs,ord,palette,scale,ncont,lev,xyDatas,rego,TEXT_to_Add,
                       col_data,image,ncat,pcex,labcex,legtext,legpos,legcex,legtitle,
                       sub,subcex,axcex,cextext,pch_dat,meth,colorkey,contourlab){
  plot.new()
  dev.off()
  len1<-length(v1)
  len2<-length(v2)
      ...
      for (s in 1:len2){
        if(colorkey==TRUE){
            ...
            if(len1==1 && len2==1){
              if(missing(tit)){
                if(pal[[1]][1]==pal[[1]][length(pal[[1]])]){
                  par(mai = c(0.75,0.75,0.75,0.75),cex=legcex)
                }
                else{
                  par(mai = c(0.75,0.75,0.75,0.75),cex=legcex)
                }
              ...
              if(missing(tit)){
                if(pal[[1]][1]==pal[[1]][length(pal[[1]])]){
                  par(mai = c(0.75,1,0.5,0.75),cex=legcex)
                }
                else{
                  par(mai = c(0.75,1,0.5,0.75),cex=legcex)
                }
                ...
                par(mai = c(0.75,1,1.5,0.75),cex=legcex)
              }
            }
          }
          else if(l==1){
          ...
          if(l==1 && s==1){
            if(len1==1 && len2==1){
              if(missing(tit)){
                if(pal[[1]][1]==pal[[1]][length(pal[[1]])]){
                  par(mai = c(0.75,1.25,0.5,0.75),cex=legcex)
                }
                else{
                  par(mai = c(0.75,1,0.5,0.75),cex=legcex)
                }
          ...
          if(l==1 && s==1){
            if(len1==1 && len2==1){
              ...
              if(missing(tit)){
                if(pal[[1]][1]==pal[[1]][length(pal[[1]])]){
                  par(mai = c(0.6,0.75,0.25,0.25),cex=legcex)
                }
                else{
                  par(mai = c(0.6,0.75,0.25,0.25),cex=legcex)
                }
                ...
                par(mai = c(0.6,0.75,1.5,0.25),cex=legcex)
              }
            }
          }
          else if(l==1){
        ...
        if(image==TRUE){
                ...
                if(pal[[1]][1]==pal[[1]][length(pal[[1]])]){
                  ...
                  image.plot(xyZ[[s+(l-1)*len2]],zlim=c(newz.na,scale[2]),col=c("light gray",palette[[1]]),xaxt="n",yaxt="n",legend.cex=legcex)
                    ...
                    image.plot(xyZ[[s+(l-1)*len2]],
                               ...
                               #breaks=seq(min(scale[1]),max(scale[2])),
                               nlevel=ncol,
                               col=c("light gray",palette[[1]]),xaxt="n",yaxt="n",axis.args=list(cex.axis=2),legend.shrink=0.5,cex.main=subcex,legend.cex=legcex)
                    }
                    else{
                      image(xyZ[[s+(l-1)*len2]],
                                 zlim=c(newz.na,scale[2]),
          ...
          if(missing(ncont)){
            ...
            if(missing(meth)){
              contour(xyZ[[s+(l-1)*len2]],nlevels=ncont, add = TRUE,labcex=1,method="edge",drawlabels = contourlab)
            }
            else{
              contour(xyZ[[s+(l-1)*len2]],nlevels=ncont, add = TRUE,labcex=1,method=meth,drawlabels = contourlab)
            }
          }
          if(missing(lev)){
          }
            ...
            if(missing(meth)){
            contour(xyZ[[s+(l-1)*len2]],levels=lev, add = TRUE,labcex=1,method="edge",drawlabels = contourlab)
            }
            else{
              contour(xyZ[[s+(l-1)*len2]],levels=lev, add = TRUE,labcex=1,method=meth,drawlabels = contourlab)
            }
          }
        }
        else{
          ...
          if(missing(ncont)){
            ...
            if(s==1 & l==1){
              if(missing(meth)){
                contour(xyZ[[s+(l-1)*len2]],nlevels=ncont,xaxt="n",yaxt="n",labcex=1,col=1,lty=4,method = "edge",drawlabels = contourlab)
                collegend[1]=2
              }
              else{
                contour(xyZ[[s+(l-1)*len2]],nlevels=ncont,xaxt="n",yaxt="n",labcex=1,col=1,lty=4,method = meth,drawlabels = contourlab)
                collegend[1]=2
              }
            }
            else{
              if(missing(meth)){
                contour(xyZ[[s+(l-1)*len2]],nlevels=ncont,add=TRUE,xaxt="n",yaxt="n",labcex=1,col=s+(l-1)*len2,lty=4,method = "edge",drawlabels = contourlab)
                collegend[s+(l-1)*len2]=1+s+(l-1)*len2
              }
              else{
                contour(xyZ[[s+(l-1)*len2]],nlevels=ncont,add=TRUE,xaxt="n",yaxt="n",labcex=1,col=s+(l-1)*len2,lty=4,method = meth,drawlabels = contourlab)
                collegend[s+(l-1)*len2]=1+s+(l-1)*len2
              }
            }
          }
          if(missing(lev)){
            ...
            if(s==1 & l==1){
              if(missing(meth)){
                contour(xyZ[[s+(l-1)*len2]],levels=lev,xaxt="n",yaxt="n",labcex=1,col=2,lty=4,method = "simple",drawlabels = contourlab)
                collegend[1]=2
              }
              else{
                contour(xyZ[[s+(l-1)*len2]],levels=lev,xaxt="n",yaxt="n",labcex=1,col=2,lty=4,method = meth,drawlabels = contourlab)
                collegend[1]=2
              }
            }
            else{
              if(missing(meth)){
                contour(xyZ[[s+(l-1)*len2]],levels=lev,add=TRUE,xaxt="n",yaxt="n",labcex=1,col=1+s+(l-1)*len2,lty=4,method = "edge",drawlabels = contourlab)
                collegend[s+(l-1)*len2]=1+s+(l-1)*len2
              }
              else{
                contour(xyZ[[s+(l-1)*len2]],levels=lev,add=TRUE,xaxt="n",yaxt="n",labcex=1,col=1+s+(l-1)*len2,lty=4,method = meth,drawlabels = contourlab)
                collegend[s+(l-1)*len2]=1+s+(l-1)*len2
              }
            }
          }
        ...
        if(missing(legtext)){
          ...
          if(image==TRUE){
            if(is.character(legpos)){
              if(missing(legtitle)){
                legend(legpos,legend=legtext,col=collegend,pch=pchlegend, cex=pcex*1.5)
              }
              else{
                legend(legpos,legend=legtext,col=collegend,pch=pchlegend,title=legtitle, cex=pcex*1.5)
              }
            }
            else{
              if(missing(legtitle)){
              legend(legpos[1],legpos[2],xpd=NA,legend=legtext,col=collegend,pch=pchlegend, cex=pcex*1.5)
              }
              else{
                legend(legpos[1],legpos[2],xpd=NA,legend=legtext,col=collegend,pch=pchlegend,title=legtitle, cex=pcex*1.5)
              }
            }
          }
          else{
            if(missing(legtitle)){
            legend(legpos,legend=legtext,col=collegend,lty=6, cex=pcex*1.5)
            }
            else{
              legend(legpos,legend=legtext,col=collegend,lty=6, cex=pcex*1.5,title=legtitle)
            }
          }
        }
        if(image==TRUE){
          ...
          if(s==1 & l==1){
            if(missing(axcex)){
              ...
              mtext(text=seq(xlim[1],xlim[2],xlim[3]),at=seq(0,1,xlim[3]/abs(xlim[2]-xlim[1])),line=5.25*axcex/3,side=1,cex=axcex)
        ...
        if(missing(labcex)){
            ...
            if(s==1 & l==1){
              if(missing(tit)){
                if(colorkey==TRUE){
                  ...
                  title(xlab=abs,line=2.5,cex.lab=1.25)
          ...
          if(image==TRUE){
            if(missing(tit)){
              if(colorkey==TRUE){
                if(s==1){
                title(ylab=ord,line=2,cex.lab=labcex)
                }
                if(l==len1){
                title(xlab=abs,line=2.5,cex.lab=labcex)
                }
              }
              else{
                if(s==1){
                title(ylab=ord,line=2.5,cex.lab=labcex)
                }
                if(l==len1){
        ...
        if (len1==1 & len2==1){
          ...
          if(l==len1){
            if(image==TRUE){
              if(colorkey=="COMMON"){
                mtext(paste(nvar2,v2[s],unitV2),side=1,line=4,cex=1,font=2)
              }
              else{
          ...
          if(s==1){
            if(image==TRUE){
              if(colorkey=="COMMON"){
              mtext(paste(nvar1,v1[l],unitV1),side=2,line=4.5,cex=1,font=2)
              }
              else{
