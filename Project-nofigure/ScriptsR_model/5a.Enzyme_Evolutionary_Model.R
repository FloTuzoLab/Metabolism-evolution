...
N_rep=30
mut_bias_set<-c(0,-0.1,-0.2,"increasing")#<-c(-0.1)
...
#for (g in 1:length(G_set)){
  ...
  G<-2*10^3*Ne##2.5*10^2#In principle 10^3
  mu<-10^-1/Ne
...
#}
  if(mut_bias=="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_bi0",3,".rda",sep=""))
  }
  if(mut_bias!="increasing"){
    save(simul,evo_results,file=paste("eq_",flux,"_Ne",Ne,"_mu",-log10(mu),"_bi0",-mut_bias*10,".rda",sep=""))
  }
