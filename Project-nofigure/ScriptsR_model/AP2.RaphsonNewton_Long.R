...
RaphsonNewton<-function(F_list,init,error){
  ...
  while((x_d/x>error | y_d/y>error) | (x_d>error | y_d>error) ){# 
    F1c<-mpfr(Fxy(F_list[[1]],x,y),120)
    #F1c<-Fxy(F_list[[1]],x,y)
    F2c<-mpfr(Fxy(F_list[[2]],x,y),120)
    #F2c<-Fxy(F_list[[2]],x,y)
    dF1_dx=mpfr(attr(dF[[1]](x,y),"gradient")[1,1],120)
    #dF1_dx=attr(dF[[1]](x,y),"gradient")[1,1]
    dF1_dy=mpfr(attr(dF[[1]](x,y),"gradient")[1,2],120)
    #dF1_dy=attr(dF[[1]](x,y),"gradient")[1,2]
    dF2_dx=mpfr(attr(dF[[2]](x,y),"gradient")[1,1],120)
    #dF2_dx=attr(dF[[2]](x,y),"gradient")[1,1]
    dF2_dy=mpfr(attr(dF[[2]](x,y),"gradient")[1,2],120)
    #dF2_dy=attr(dF[[2]](x,y),"gradient")[1,2]
    y_d=mpfr((-F2c+F1c*dF2_dx/dF1_dx)/(dF2_dy-dF2_dx*dF1_dy/dF1_dx),120)
    #y_d=(-F2c+F1c*dF2_dx/dF1_dx)/(dF2_dy-dF2_dx*dF1_dy/dF1_dx)
    x_d=mpfr((-F1c-y_d*dF1_dy)/dF1_dx,120)
    #x_d=(-F1c-y_d*dF1_dy)/dF1_dx
    x=as.numeric(mpfr(x+x_d,120))
    #x=x+x_d
    y=as.numeric(mpfr(y+y_d,120))
    #y=y+y_d
    i=i+1
  }
  res<-list("x"=x,"y"=y)
